...
// ── Fotos desde Supabase Storage bucket "Ejecutivo" ───────────────────────────
const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
const BUCKET_URL = `${SUPABASE_URL}/storage/v1/object/public/Ejecutivo`;
...
const INDICATORS = [
  { label: "Inflación (m.a.m)", value: "13.2%", trend: "down", color: "text-emerald-600", badge: "-7.4%" },
  { label: "Tasa de Pobreza",   value: "41.7%", trend: "up",   color: "text-red-600",     badge: "+1.5%" },
  { label: "Crecimiento PBI",   value: "-1.6%", trend: "up",   color: "text-red-600",     badge: "-0.2%" },
];

const CABINET = [
  { id: "1",  name: "Manuel Adorni",              ministry: "Jefatura de Gabinete",    photo: img("Manuel_Adorni_-_Retrato_Oficial_2024.png") },
  { id: "2",  name: "Luis Andrés Caputo",         ministry: "Ministerio de Economía",  photo: img("Caputocepo_(cropped).jpg") },
  { id: "3",  name: "Sandra Pettovello",          ministry: "Min. Capital Humano",     photo: img("Pettovellotorres2_(cropped).jpg") },
  { id: "4",  name: "Alejandra Monteoliva",       ministry: "Ministerio de Seguridad", photo: img("alejandramoteoliva.jpg") },
  { id: "5",  name: "Carlos Alberto Presti",      ministry: "Ministerio de Defensa",   photo: img("Albertopresti.jpg") },
  { id: "6",  name: "Diego César Santilli",       ministry: "Ministerio del Interior", photo: img("SantilliOficial.jpg") },
  { id: "7",  name: "Juan Bautista Mahiques",     ministry: "Ministerio de Justicia",  photo: img("Juan_Bautista_Mahiques,_Fiscal_General_de_la_ciudad_de_Buenos_Aires.jpg") },
  { id: "8",  name: "Pablo Quirno Magrane",       ministry: "Relaciones Exteriores",   photo: img("Pablo_Quirno_Magrane_-_Retrato_Oficial_2025.png") },
  { id: "9",  name: "Mario Iván Lugones",         ministry: "Ministerio de Salud",     photo: img("Mario_Lugones_(2025)_(cropped).jpg") },
  { id: "10", name: "Federico Sturzenegger",      ministry: "Min. de Desregulación",   photo: img("FedericoSturzeneggerOficial.jpg") },
];
...
const RECENT_ACTIONS = [
  {
    type: "DNU", typeColor: "bg-purple-100 text-purple-700",
    title: "Revisión Ley Ómnibus: Sección IV",
    date: "Oct 24, 2024",
    description: "Enmiendas a regulaciones del sector energético destinadas a aumentar cuotas de inversión y liberalizar acuerdos comerciales transfronterizos.",
    status: "Vigente", statusColor: "text-emerald-600",
  },
  {
    type: "Decreto", typeColor: "bg-blue-100 text-blue-700",
    title: "Expansión del Protocolo de Seguridad Federal",
    date: "Oct 21, 2024",
    description: "Implementación de nuevas tecnologías de vigilancia en nodos de transporte de alto tráfico.",
    status: "Implementando", statusColor: "text-amber-600",
  },
  {
    type: "Resolución", typeColor: "bg-green-100 text-green-700",
    title: "Reforma Previsional — Ajuste de Haberes",
    date: "Oct 18, 2024",
    description: "Actualización trimestral de haberes jubilatorios según índice de inflación.",
    status: "Aprobado", statusColor: "text-emerald-600",
  },
];

const CONGRESS_RELATION = { diputados: 68, senado: 45, aprobados: 82 };
...
function Avatar({ src, name, size = "md" }: { src?: string; name: string; size?: "sm" | "md" | "lg" }) {
  ...
  return src ? (
    <img src={src} alt={name} className={`${sz} rounded-full object-cover border-2 border-white/40 shadow-sm`} />
  ) : (
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Últimas voces + foco discursos + congreso */}
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <h3 className="font-serif text-xl font-bold text-gray-900 px-1">Rel. con el Congreso</h3>
            <div className="glass-card-dash p-5 rounded-3xl flex flex-col justify-between h-56">
              ...
              <div className="pt-3 border-t border-black/5">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Estado: Polarizado</p>
                </div>
                <Link href="/dashboard/congress" className="text-[10px] text-gray-400 hover:text-gray-800 flex items-center gap-1 mt-1 transition-colors">
                  Ver actividad <span className="material-symbols-outlined text-[12px]">arrow_forward</span>
                </Link>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="glass-card-dash p-5 rounded-3xl h-56 space-y-4 flex flex-col justify-center">
              {SPEECH_TOPICS.map(t => (
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => cabinetRef.current?.scrollBy({ left: -320, behavior: "smooth" })}
        ...
        <div ref={cabinetRef} className="flex gap-4 overflow-x-auto pb-4" style={{ scrollbarWidth: "none" }}>
          {CABINET.map((m, i) => (
            <div
              key={m.id}
              className={`flex-none w-56 glass-card-dash p-6 rounded-3xl flex flex-col items-center text-center transition-transform hover:scale-[1.02] ${i === 0 ? "border-l-4 border-gray-900" : ""}`}
            >
              <img
                src={m.photo}
                alt={m.name}
                className="w-16 h-16 rounded-full object-cover border-2 border-white/40 shadow-sm grayscale hover:grayscale-0 transition-all"
              />
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-4 space-y-4">
            {RECENT_ACTIONS.slice(1).map((a, i) => (
              <div key={i} className="glass-card-dash p-6 rounded-3xl">
